...
def ActuallyLoad():
    ...
    if not basename:
        print('Preset empty: {:d}'.format(preset))
        display("E{:03d}".format(preset))
        return
    print('basename:', basename, 'dirname:', dirname)
    print('Preset loading: {:d} ({:s})'.format(preset, basename))
        ...
        with open(definitionfname, 'r') as definitionfile:
            for i, pattern in enumerate(definitionfile):
                try:
                    ...
                    print("Error in definition file, skipping line {:d}.".format(i+1))
...
try:
    ...
    sd.start()
    print('Opened audio device #{:d}'.format(AUDIO_DEVICE_ID))
except:
    print('Invalid audio device #{:d}'.format(AUDIO_DEVICE_ID))
    exit(1)
